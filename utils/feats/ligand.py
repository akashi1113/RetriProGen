...
def splitSmi(smi):
    ...
    return tokens
def fetchIndices(smiSplit, smiVoc, smiMaxLen,name):
    smiIndices = []
...
def parse_sdf_file(config,path):
    ...
    smiArr_input = splitSmi(a)
    smiArr_tgt = splitSmi(a)

    smiIndices_input = fetchIndices(smiArr_input, config.model.decoder.smiVoc, 200, 'input')
    smiIndices_tgt = fetchIndices(smiArr_tgt, config.model.decoder.smiVoc, 200, 'tgt')
    ...
    data = {
        ...
        'weight': weight,
        'tpsa': tpsa,
        'smiIndices_input': torch.tensor(smiIndices_input),
        'smiIndices_tgt': torch.tensor(smiIndices_tgt),
    }
    return data
