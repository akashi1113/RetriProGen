...
class QVinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, recpter_path, ligand_rdmol, ref_sdf, tmp_dir='./tmp', use_uff=True,
                 center=None):
        super().__init__(recpter_path, ligand_rdmol)
        ...
        self.receptor_path = recpter_path
        ...
        self.protein_name = '/'.join(self.receptor_path.split('/')[-2:])
...
/home/aichengwei/package/mgltools_x86_64Linux2_1.5.6/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py -r {receptor_id}.pdb -o {receptor_id}.pdbqt
# Prepare ligand
...
obabel {ligand_id}_out.pdbqt -O{ligand_id}_out.sdf -h
        """.format(
            receptor_id=self.receptor_path,
    ...
    def run_sync(self):
        ...
        if len(results) == 0:
            ...
            # print(f'protein: {self.protein_name} Best affinity:', results[0]['affinity'])
