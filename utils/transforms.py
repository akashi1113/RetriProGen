...
class RefineData(object):
    ...
    def __call__(self, data):
        ...
        if torch.sum(is_H_ligand) > 0:
            ...
            old_ligand_bond_index = data.ligand_bond_index[:, ind_bond_without_H].to(torch.long)
            # old_ligand_bond_index = torch.tensor(old_ligand_bond_index, dtype=torch.long)
            data.ligand_bond_index = torch.tensor(index_changer,dtype=torch.long)[old_ligand_bond_index]
...
class LigandCountNeighbors(object):
    ...
    def count_neighbors(edge_index, symmetry, valence=None, num_nodes=None):
        ...
        return scatter_add(valence, index=edge_index[0].to(dtype=torch.int64), dim=0, dim_size=num_nodes).long()
...
class FocalBuilder(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if has_unmask_atoms:
            ...
            idx_generated_in_ligand_masked = index_changer_masked[idx_generated_in_whole_ligand.type(torch.long)]
            pos_generate = ligand_masked_pos[idx_generated_in_ligand_masked.type(torch.long)]
            ...
            idx_focal_in_ligand_context = index_changer_context[idx_focal_in_whole_ligand.long()]
...
class Res2AtomComposer(object):  #protein_dim=26, ligand_dim=13
    ...
    def get_knn_graph(data, knn, len_ligand_ctx, len_compose, num_workers=1):
        ...
        data.compose_knn_edge_type = torch.zeros(len(data.compose_knn_edge_index[0]), dtype=torch.long)  # for encoder.py edge embedding
        data.compose_knn_edge_type[idx_edge[idx_edge>=0]] = data.ligand_context_bond_type[idx_edge>=0].long()
        ...
        data.compose_knn_edge_feature[idx_edge[idx_edge>=0]] = F.one_hot(data.ligand_context_bond_type[idx_edge>=0].long(), num_classes=4)    # 0 (1,2,3)-onehot
        return data
...
class EdgeSample(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if has_unmask_atoms:
            ...
            new_edge_index_0 = index_changer_masked[candidate_bond_index[0].long()]
            new_edge_index_1 = index_changer_context[candidate_bond_index[1].long()]
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if data.ligand_bond_index.size(1) != 0:
            data.ligand_context_bond_index, data.ligand_context_bond_type = subgraph(
                context_idx,
                data.ligand_bond_index.type(torch.long),
                edge_attr = data.ligand_bond_type.type(torch.long),
                relabel_nodes = True,
            )
        else:
...
class LigandBFSMask(object):
    ...
    def __call__(self, data):
        ...
        if data.ligand_bond_index.size(1) != 0:
            data.ligand_context_bond_index, data.ligand_context_bond_type = subgraph(
                context_idx.to(dtype=torch.int64)
,
                data.ligand_bond_index.to(dtype=torch.int64),
...
class ContrastiveSample(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        adj_mat[data.ligand_context_bond_index[0], data.ligand_context_bond_index[1]] = data.ligand_context_bond_type.long()
