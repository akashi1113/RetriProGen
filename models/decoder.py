...
class Decoder(Module):
    ...
    def forward(self, smiles_index, enc_outputs, tgt_len, prop=None, device='cpu'):
        ...
        if self.num_props:
            ...
            # type_embeddings = self.type_emb(torch.ones((b,t), dtype = torch.long))
            dec_inputs = dec_inputs + type_embeddings
            # type_embd = self.type_emb(torch.zeros((b, 1), dtype = torch.long))
        ...
        dec_enc_attn_mask = torch.ones((b, tgt_len + num, enc_outputs.size(1)), dtype=torch.bool).to(device)
