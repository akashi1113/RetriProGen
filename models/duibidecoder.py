...
class DecoderLayer(Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask ,dec_enc_attn_mask):
        ...
        dec_outputs = self.dec_enc_attn(dec_outputs.to(enc_outputs.device), enc_outputs, enc_outputs, dec_enc_attn_mask)
...
class Decoder(Module):
    ...
    def forward(self, smiles_index,enc_outputs, tgt_len,prop = None):
        ...
        if self.num_props:
            ...
            # type_embeddings = self.type_emb(torch.ones((b,t), dtype = torch.long).to(enc_outputs.device))
            dec_inputs = dec_inputs + type_embeddings
            type_embd = self.type_emb(torch.zeros((b, 1), dtype=torch.long).to(self.type_emb.weight.device))

            p = self.prop_nn(prop.unsqueeze(1))
            p += type_embd
            dec_inputs = torch.cat([p, dec_inputs], 1)

            # con = torch.ones(smiles_index.shape[0],1)
        ...
        dec_self_attn_pad_mask = dec_self_attn_pad_mask.to(dec_self_attn_subsequent_mask.device)
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask + dec_self_attn_subsequent_mask), 0)
        #下面这一行报错
        dec_enc_attn_mask = torch.ones((b, tgt_len + num, enc_outputs.size(1)), dtype=torch.bool)
        # dec_enc_attn_mask = torch.ones((b, tgt_len+num, enc_outputs.size(1)), dtype=torch.bool)
